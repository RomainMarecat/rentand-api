...
use Tests\Component\Validator\Constraints\ImageValidator;
use Entity\Image;
...
class ImageTest extends \PHPUnit_Framework_TestCase
{
    ...
    /**
     * @var Tests\Component\Validator\Constraints\ImageValidator
     */
    private $validator;
    ...
    {
        $this->assertInstanceOf('Entity\Image', $this->entity);
    }
    ...
    public function testValidateAlt()
    {
        $this->constraint = new \Tests\Component\Validator\Type\StringType();
        $this->context->expects($this->exactly($this->constraint->getNbInvalidate()))
            ->method('addViolation')
        ...
        foreach ($this->constraint->getValues()['string'] as $key => $value) {
            if ($key >= 1) {
                $this->entity->setAlt($value['value']);
                $this->validator->validate($this->entity->getAlt(), $this->constraint);
            }
        }
    }
